...
You will need sintruments library (https://github.com/tiagocoutinho/sinstruments)
to run the simulator. You can install it easily with `pip install sinstruments`.

To create a Hamamatsu device use the following configuration as a starting
point:
...
.. code-block:: yaml
    ...
    devices:
    - class: RemoteEx
      package: hamamatsu.simulator
      data_port: 11002
      transports:
      - type: tcp
        url: :11001

Save this configuration into a file called `hamamatsu.yml`. You can start the
simulator with:

    $ python -m hamamatsu.simulator -c ./hamamatsu.yml
...
AppInfo = collections.namedtuple(
    "AppInfo",
    ("type", "date", "version", "directory", "title", "titlelong", "progdatadir"),
)
...
ParamInfo = collections.namedtuple("ParamInfo", ("label", "type", "default", "access"))
...
AcqParamInfo = {
    ...
    "deactivategrbnotituse": ParamInfoRW(
        "Deactivate grabber not in use", ParamType.Boolean, 0
    ),
...
CamParamInfo = {
    "setup": {
        ...
        "showgainoffset": ParamInfoRW(
            "Show Gain and Offset on acquisition dialog", ParamType.Text
        ),
        ...
        "scrollinglivedisplay": ParamInfoRW(
            "Scrolling or non scrolling live display", ParamType.Text
        ),
        "frametrigger": ParamInfoRW(
            "Frame trigger (TDI or X-ray line sensors)", ParamType.Text
        ),
        ...
        "mechanicalshutter": ParamInfoRW(
            "Behavior of Mechanical Shutter", ParamType.Text
        ),
        "subtype": ParamInfoRW("Subtype (X-Ray Flatpanel)", ParamType.Text),
        "autodetect": ParamInfoRW("Auto detect subtype", ParamType.Text),
        "wait2ndframe": ParamInfoRW(
            "Wait for second frame in Acquire mode", ParamType.Text
        ),
        ...
        "displayduringmeasurement": ParamInfoRW(
            "Display during measurement option", ParamType.Text
        ),
        "gaintable": ParamInfoRW("Gain table", ParamType.Text),
        "nooftimestocheck": ParamInfoRW("Number of times to check", ParamType.Text),
        "maximumbackgroundlevel": ParamInfoRW(
            "Maximum background level", ParamType.Text
        ),
        "minimumsensitivitylevel": ParamInfoRW(
            "Maximum sensitivity level", ParamType.Text
        ),
        "fluctuation": ParamInfoRW("Fluctuation", ParamType.Text),
        "noofintegration": ParamInfoRW("Number of Integration", ParamType.Text),
        "dualenergycorrection": ParamInfoRW(
            "Dual energy correction method", ParamType.Text
        ),
        "lowenergyvalue": ParamInfoRW(
            "Dual energy correction low energy value", ParamType.Text
        ),
        "highenergyvalue": ParamInfoRW(
            "Dual energy correction high energy value", ParamType.Text
        ),
        ...
        "twopcthreshold": ParamInfoRW(
            "Use two thresholds instead of one", ParamType.Text
        ),
        "automaticbundleheight": ParamInfoRW(
            "Use automatic calculation of bundle height", ParamType.Text
        ),
        ...
        "serialout": ParamInfoRW(
            "Programming of the Serial Out string", ParamType.Text
        ),
    ...
    "acquire": {
        ...
        "dortbacksub": ParamInfoRW(
            "Do realtime background subtraction", ParamType.Text
        ),
        ...
        "lightmode": ParamInfoRW("Light mode", ParamType.Text),
        "scanspeed": ParamInfoRW("Scan Speed", ParamType.Text),
        "bgdatamemory": ParamInfoRW(
            "Memory number for background data (Inbuilt background sub)", ParamType.Text
        ),
        "shdatamemory": ParamInfoRW(
            "Memory number for shading data (Inbuilt shading correction)",
            ParamType.Text,
        ),
        ...
        "recurnumber2": ParamInfoRW(
            "Second number for recursive filter", ParamType.Text
        ),
...
def parse_config_params(config, name, group):
    data = config.get(name, {})
    result = {}
    for k, v in group.items():
        info = group[k]
        result[k] = Param(data.get(k, info.default), info)
    return result
...
class RemoteEx(BaseDevice):
    ...
    def __init__(self, name, **opts):
        ...
        super(RemoteEx, self).__init__(name, **super_kwargs)
        self._app = parse_config_params(opts, "application", AppParamInfo)
        self._main = parse_config_params(opts, "main", MainParamInfo)
        self._acq = parse_config_params(opts, "acquisition", AcqParamInfo)
        self._cam = {}
        for gname, gparams in CamParamInfo.items():
            gopts = opts.get(gname, {})
            self._cam[gname] = parse_config_params(gopts, gname, gparams)
        self._status = "idle"
        self._started = False
        self._acq_mode = None
        self._commands = cmds = {}
